...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'LoopEntryFixture.cs') -Destination $inputDirectory
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'EmitLoopEntry.cs') -Destination $emitterDirectory
'<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFramework>net48</TargetFramework><OutputType>Exe</OutputType><Optimize>true</Optimize></PropertyGroup></Project>' |
    Set-Content (Join-Path $inputDirectory 'LoopEntryFixture.csproj')
...
dotnet build (Join-Path $inputDirectory 'LoopEntryFixture.csproj') -c Release --nologo -v quiet
if ($LASTEXITCODE -ne 0) { throw 'Fixture build failed.' }
$inputExe = Join-Path $OutputDirectory 'LoopEntryFixture.exe'
dotnet run --project (Join-Path $emitterDirectory 'Emitter.csproj') -c Release -- (Join-Path $inputDirectory 'bin\Release\net48\LoopEntryFixture.exe') $inputExe
if ($LASTEXITCODE -ne 0) { throw 'Loop entry metadata generation failed.' }
$hash = (Get-FileHash -LiteralPath $inputExe).Hash
& $inputExe
if ($LASTEXITCODE -ne 0) { throw 'Original loop entry state behavior failed.' }
foreach ($threads in @(1,4)) {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'Loop entry export failed.' }
    $watch.Stop()
    ...
    if ($LASTEXITCODE -ne 0) { throw 'Exported loop entry source compilation failed.' }
    & (Join-Path $rebuilt 'LoopEntryFixture.exe')
    if ($LASTEXITCODE -ne 0) { throw 'Rebuilt loop entry state behavior changed.' }
}
