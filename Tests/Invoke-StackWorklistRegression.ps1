...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'StackWorklistFixture.cs') -Destination $inputDirectory
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'EmitStackWorklist.cs') -Destination $emitterDirectory
'<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFramework>net48</TargetFramework><OutputType>Exe</OutputType><Optimize>true</Optimize></PropertyGroup></Project>' |
    Set-Content (Join-Path $inputDirectory 'StackWorklistFixture.csproj')
...
dotnet build (Join-Path $inputDirectory 'StackWorklistFixture.csproj') -c Release --nologo -v quiet
if ($LASTEXITCODE -ne 0) { throw 'Fixture build failed.' }
$inputExe = Join-Path $OutputDirectory 'StackWorklistFixture.exe'
dotnet run --project (Join-Path $emitterDirectory 'Emitter.csproj') -c Release -- (Join-Path $inputDirectory 'bin\Release\net48\StackWorklistFixture.exe') $inputExe
if ($LASTEXITCODE -ne 0) { throw 'Dispatcher metadata generation failed.' }
$hash = (Get-FileHash -LiteralPath $inputExe).Hash
& $inputExe
if ($LASTEXITCODE -ne 0) { throw 'Original dispatcher state behavior failed.' }
foreach ($threads in @(1,4)) {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'Dispatcher export failed.' }
    $watch.Stop()
    ...
    if ($LASTEXITCODE -ne 0) { throw 'Exported dispatcher source compilation failed.' }
    & (Join-Path $rebuilt 'StackWorklistFixture.exe')
    if ($LASTEXITCODE -ne 0) { throw 'Rebuilt dispatcher state behavior changed.' }
}
