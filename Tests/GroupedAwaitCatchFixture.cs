...
public static class GroupedAwaitCatchFixture {
    ...
    static Exception Error(int mode) => mode == 1 || mode == 2 ? (Exception)new GroupedFailure() : mode == 3 ?
        new TimeoutException("timeout") : mode == 5 ? new OperationCanceledException(new CancellationToken(true)) : new FormatException("other");
    static void Complete(TaskCompletionSource<int> gate, int mode, Exception error) {
        if (mode >= 2 && mode <= 5) gate.SetException(error); else gate.SetResult(17);
    }
    static int Mode(int[] modes, int phase) => modes[phase / 4 * 2 + (phase % 4 == 0 ? 0 : 1)];
    public static int Main() {
        var method = new DynamicMethod("ThrowPayload", typeof(void), new[] { typeof(object) }, typeof(GroupedAwaitCatchFixture).Module);
        ...
        for (int code = 0; code < 2401; code++) foreach (int delayed in new[] { 0, 15, 240, 255 }) {
            cases++;
            int[] modes = { code % 7, code / 7 % 7, code / 49 % 7, code / 343 };
        ...
        Console.WriteLine("PASS: " + cases + " grouped catch cases / " + checks + " assertions.");
        return 0;
    }
}
