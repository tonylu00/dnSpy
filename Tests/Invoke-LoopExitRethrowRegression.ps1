...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'LoopExitRethrowFixture.cs') -Destination $inputDirectory
'<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFramework>net48</TargetFramework><OutputType>Exe</OutputType><Optimize>true</Optimize><LangVersion>latest</LangVersion></PropertyGroup></Project>' | Set-Content (Join-Path $inputDirectory 'LoopExitRethrowFixture.csproj')
dotnet build $inputDirectory -c Release --nologo -v quiet
if($LASTEXITCODE -ne 0){throw 'Loop catch fixture build failed.'}
$original=Join-Path $inputDirectory 'bin\Release\net48\LoopExitRethrowFixture.exe'
$hash=(Get-FileHash -LiteralPath $original).Hash
$expected=@(& $original)
if($LASTEXITCODE -ne 0){throw 'Original loop catch behavior failed.'}
...
$inputAssembly=Join-Path $variant 'LoopExitRethrowFixture.exe'
dotnet run --project (Join-Path $emitter 'Emitter.csproj') -c Release -- $original $inputAssembly Retry
if($LASTEXITCODE -ne 0){throw 'Loop rethrow emission failed.'}
...
foreach($threads in @(1,4)) {
    ...
    if($LASTEXITCODE -ne 0){throw 'Loop rethrow export failed.'}
    $source=Get-Content (Join-Path $export 'LoopExitRethrowFixture\LoopExitRethrowFixture.cs') -Raw
    $project=Get-ChildItem -LiteralPath $export -Recurse -Filter '*.csproj' | Select-Object -First 1
    $rebuilt=Join-Path $OutputDirectory "rebuilt-$threads"
    dotnet build $project.FullName -c Release -o $rebuilt --nologo -v quiet
    if($LASTEXITCODE -ne 0){throw 'Loop rethrow source compilation failed.'}
    if($source -match 'catch \(object' -or $source -match 'ExceptionDispatchInfo.Capture'){throw 'Captured loop catch rethrow was not recovered.'}
    $actual=@(& (Join-Path $rebuilt 'LoopExitRethrowFixture.exe'))
    if($LASTEXITCODE -ne 0){throw 'Rebuilt loop catch behavior failed.'}
    $actual | Set-Content (Join-Path $OutputDirectory "actual-$threads.txt")
    if(Compare-Object $expected $actual -SyncWindow 0 -CaseSensitive){throw 'Loop catch behavior changed.'}
}
$debug=Join-Path $OutputDirectory 'debug'
New-Item -ItemType Directory -Path $debug | Out-Null
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'LoopExitRethrowDebug.cs') -Destination $debug
...
if($LASTEXITCODE -ne 0){throw 'Loop rethrow safety/debug checks failed.'}
