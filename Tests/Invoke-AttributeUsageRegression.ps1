...
foreach($pair in @(@('AttributeUsageLibrary.cs','library'),@('AttributeUsageClient.cs','client'),@('EmitAttributeUsage.cs','emitter'))){Copy-Item (Join-Path $PSScriptRoot $pair[0]) (Join-Path $OutputDirectory $pair[1])}
'<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFramework>net48</TargetFramework><Optimize>true</Optimize></PropertyGroup></Project>' | Set-Content (Join-Path $OutputDirectory 'library\AttributeUsageLibrary.csproj')
'<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFramework>net48</TargetFramework><OutputType>Exe</OutputType><Optimize>true</Optimize></PropertyGroup><ItemGroup><ProjectReference Include="..\library\AttributeUsageLibrary.csproj" /></ItemGroup></Project>' | Set-Content (Join-Path $OutputDirectory 'client\AttributeUsageClient.csproj')
...
dotnet build (Join-Path $OutputDirectory 'client\AttributeUsageClient.csproj') -c Release --nologo -v quiet
if($LASTEXITCODE -ne 0){throw 'Input build failed.'}
$inputDir=Join-Path $OutputDirectory 'input'
dotnet run --project (Join-Path $OutputDirectory 'emitter\Emitter.csproj') -c Release -- (Join-Path $OutputDirectory 'library\bin\Release\net48\AttributeUsageLibrary.dll') (Join-Path $OutputDirectory 'client\bin\Release\net48\AttributeUsageClient.exe') $inputDir
if($LASTEXITCODE -ne 0){throw 'Metadata emission failed.'}
$exe=Join-Path $inputDir 'AttributeUsageClient.exe'; $library=Join-Path $inputDir 'AttributeUsageLibrary.dll'
$hashes=@(Get-FileHash $exe,$library | ForEach-Object Hash)
& $exe
if($LASTEXITCODE -ne 0){throw 'Original attribute applications failed.'}
foreach($threads in 1,4){
 ...
 $project=Get-ChildItem $export -Recurse -Filter AttributeUsageClient.csproj | Select-Object -First 1
 $rebuilt=Join-Path $OutputDirectory "rebuilt-$threads"
 dotnet build $project.FullName -c Release -o $rebuilt --nologo -v quiet
 if($LASTEXITCODE -ne 0){throw 'Rebuilt source failed.'}
 & (Join-Path $rebuilt 'AttributeUsageClient.exe')
...
foreach($relative in 'AttributeUsageLibrary\MarkAttribute.cs','AttributeUsageClient\AttributeUsageClient.cs') {
...
Write-Host 'PASS: attribute usage export, rebuild, runtime and deterministic source'
