...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'ObservedSelectorCatchFixture.cs') -Destination $inputDirectory
'<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFramework>net48</TargetFramework><OutputType>Exe</OutputType><Optimize>true</Optimize><LangVersion>latest</LangVersion></PropertyGroup></Project>' | Set-Content (Join-Path $inputDirectory 'ObservedSelectorCatchFixture.csproj')
dotnet build $inputDirectory -c Release --nologo -v quiet
if($LASTEXITCODE -ne 0){throw 'Catch fixture build failed.'}
$original=Join-Path $inputDirectory 'bin\Release\net48\ObservedSelectorCatchFixture.exe'
...
foreach($threads in @(1,4)) {
    ...
    if($LASTEXITCODE -ne 0){throw 'Observed selector catch rethrow export failed.'}
    $source=Get-Content (Join-Path $export 'ObservedSelectorCatchFixture\ObservedSelectorCatchFixture.cs') -Raw
    ...
    if($LASTEXITCODE -ne 0){throw 'Observed selector catch rethrow source compilation failed.'}
    $actual=@(& (Join-Path $rebuilt 'ObservedSelectorCatchFixture.exe'))
    if($LASTEXITCODE -ne 0){throw 'Rebuilt observed selector catch behavior failed.'}
    $actual | Set-Content (Join-Path $OutputDirectory "actual-$threads.txt")
    if(Compare-Object $expected $actual -SyncWindow 0 -CaseSensitive){throw 'Observed selector catch behavior changed.'}
}
$debug=Join-Path $OutputDirectory 'debug'
New-Item -ItemType Directory -Path $debug | Out-Null
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'ObservedSelectorCatchDebug.cs') -Destination $debug
...
if($LASTEXITCODE -ne 0){throw 'Observed selector catch rethrow safety/debug checks failed.'}
