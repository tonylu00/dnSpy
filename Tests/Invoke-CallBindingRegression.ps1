...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'CallBindingFixture.cs') -Destination $inputDirectory
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'EmitCallBinding.cs') -Destination $emitter
'<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFramework>net48</TargetFramework><OutputType>Exe</OutputType><Optimize>true</Optimize><LangVersion>latest</LangVersion></PropertyGroup></Project>' | Set-Content (Join-Path $inputDirectory 'CallBindingFixture.csproj')
...
dotnet build (Join-Path $inputDirectory 'CallBindingFixture.csproj') -c Release -p:PlatformTarget=$Platform --nologo -v quiet
if($LASTEXITCODE -ne 0){throw 'Call binding fixture compilation failed.'}
$original=Join-Path $OutputDirectory 'CallBindingFixture.exe'
dotnet run --project (Join-Path $emitter 'Emitter.csproj') -c Release -- (Join-Path $inputDirectory 'bin\Release\net48\CallBindingFixture.exe') $original
if($LASTEXITCODE -ne 0){throw 'Call binding emission failed.'}
$expected=@(& $original)
if($LASTEXITCODE -ne 0){throw 'Emitted call binding behavior failed.'}
...
foreach($threads in @(1,4)) {
    ...
    if($LASTEXITCODE -ne 0){throw 'Call binding export failed.'}
    $project=Get-ChildItem -LiteralPath $export -Recurse -Filter '*.csproj' | Select-Object -First 1
    $rebuilt=Join-Path $OutputDirectory "rebuilt-$threads"
    dotnet build $project.FullName -c Release -o $rebuilt -p:PlatformTarget=$Platform --nologo -v quiet
    if($LASTEXITCODE -ne 0){throw 'Call binding source compilation failed.'}
    $actual=@(& (Join-Path $rebuilt 'CallBindingFixture.exe'))
    if($LASTEXITCODE -ne 0){throw 'Call binding source behavior failed.'}
    $actual | Set-Content (Join-Path $OutputDirectory "actual-$threads.txt")
    if(Compare-Object $expected $actual -SyncWindow 0 -CaseSensitive){throw 'Call binding results differ.'}
}
$debug=Join-Path $OutputDirectory 'debug'
New-Item -ItemType Directory -Path $debug | Out-Null
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'CallBindingDebug.cs') -Destination $debug
...
if($LASTEXITCODE -ne 0){throw 'Call binding debug validation failed.'}
