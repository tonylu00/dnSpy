...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'ClosureContextFixture.cs') -Destination $source
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'EmitClosureContext.cs') -Destination $emitter
'<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFramework>net48</TargetFramework><OutputType>Exe</OutputType><Optimize>true</Optimize><LangVersion>latest</LangVersion></PropertyGroup></Project>' | Set-Content (Join-Path $source 'ClosureContextFixture.csproj')
...
dotnet build (Join-Path $source 'ClosureContextFixture.csproj') -c Release --nologo -v quiet
if ($LASTEXITCODE -ne 0) { throw 'Closure context fixture compilation failed.' }
$original = Join-Path $source 'bin\Release\net48\ClosureContextFixture.exe'
$inputExe = Join-Path $OutputDirectory 'ClosureContextFixture.exe'
$expected = @(& $original)
if ($LASTEXITCODE -ne 0) { throw 'Original closure context behavior failed.' }
...
if ($LASTEXITCODE -ne 0) { throw 'Closure context emission failed.' }
$emitted = @(& $inputExe)
if ($LASTEXITCODE -ne 0 -or (Compare-Object $expected $emitted -SyncWindow 0 -CaseSensitive)) { throw 'Renamed closure context behavior differs.' }
...
foreach ($threads in @(1,4)) {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'Closure context export failed.' }
    ...
    if ($LASTEXITCODE -ne 0) { throw 'Closure context source compilation failed.' }
    $actual = @(& (Join-Path $rebuilt 'ClosureContextFixture.exe'))
    if ($LASTEXITCODE -ne 0 -or (Compare-Object $expected $actual -SyncWindow 0 -CaseSensitive)) { throw 'Closure context source behavior differs.' }
...
Write-Output 'PASS: closure context source and runtime regression.'
