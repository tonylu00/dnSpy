...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'EmitBranchedFinallyReuse.cs') -Destination $emitter
...
if($LASTEXITCODE -ne 0){throw 'Branched finally rethrow emission failed.'}
...
foreach($threads in @(1,4)) {
    ...
    if($LASTEXITCODE -ne 0){throw 'Branched finally rethrow export failed.'}
    ...
    if($LASTEXITCODE -ne 0){throw 'Branched finally rethrow source compilation failed.'}
    $actual=@(& (Join-Path $rebuilt 'NestedAwaitFinallyFixture.exe'))
    if($LASTEXITCODE -ne 0){throw 'Rebuilt branched finally cleanup behavior failed.'}
    $actual | Set-Content (Join-Path $OutputDirectory "actual-$threads.txt")
    if(Compare-Object $expected $actual -SyncWindow 0 -CaseSensitive){throw 'Branched finally cleanup behavior changed.'}
}
$debug=Join-Path $OutputDirectory 'debug'
New-Item -ItemType Directory -Path $debug | Out-Null
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'BranchedFinallyReuseDebug.cs') -Destination $debug
...
if($LASTEXITCODE -ne 0){throw 'Branched finally rethrow safety/debug checks failed.'}
