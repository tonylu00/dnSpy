...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'ConstructorInitializerFixture.cs') -Destination $inputDirectory
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'EmitConstructorInitializer.cs') -Destination $emitterDirectory
'<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFramework>net48</TargetFramework><OutputType>Exe</OutputType><Optimize>true</Optimize><LangVersion>latest</LangVersion></PropertyGroup></Project>' | Set-Content (Join-Path $inputDirectory 'ConstructorInitializerFixture.csproj')
...
dotnet build (Join-Path $inputDirectory 'ConstructorInitializerFixture.csproj') -c Release --nologo -v quiet
if($LASTEXITCODE -ne 0){throw 'Constructor initializer fixture build failed.'}
$source=Join-Path $inputDirectory 'bin\Release\net48\ConstructorInitializerFixture.exe'
$original=Join-Path $OutputDirectory 'ConstructorInitializerFixture.exe'
dotnet run --project (Join-Path $emitterDirectory 'Emitter.csproj') -c Release -- $source $original
if($LASTEXITCODE -ne 0){throw 'Constructor initializer renaming failed.'}
& $original
if($LASTEXITCODE -ne 0){throw 'Original constructor initializer behavior failed.'}
foreach($threads in @(1,4)) {
    $export=Join-Path $OutputDirectory "export-$threads"
    & $DnSpyConsole --no-color --sdk-project --threads $threads -o $export $original
    if($LASTEXITCODE -ne 0){throw 'Constructor initializer export failed.'}
    $project=Get-ChildItem -LiteralPath $export -Recurse -Filter '*.csproj' | Select-Object -First 1
    $rebuilt=Join-Path $OutputDirectory "rebuilt-$threads"
    dotnet build $project.FullName -c Release -o $rebuilt --nologo -v quiet
    if($LASTEXITCODE -ne 0){throw 'Constructor initializer source compilation failed.'}
    & (Join-Path $rebuilt 'ConstructorInitializerFixture.exe')
    if($LASTEXITCODE -ne 0){throw 'Rebuilt constructor initializer behavior changed.'}
}

$debug=Join-Path $OutputDirectory 'debug'
New-Item -ItemType Directory -Path $debug | Out-Null
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'ConstructorInitializerDebug.cs') -Destination $debug
...
if($LASTEXITCODE -ne 0){throw 'Constructor initializer debug transformation failed.'}
