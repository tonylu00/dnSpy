...
Copy-Item (Join-Path $PSScriptRoot 'StackCounterFixture.cs') (Join-Path $OutputDirectory 'fixture')
Copy-Item (Join-Path $PSScriptRoot 'EmitStackCounter.cs') (Join-Path $OutputDirectory 'emitter')
'<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFramework>net48</TargetFramework><OutputType>Exe</OutputType><Optimize>true</Optimize></PropertyGroup></Project>' | Set-Content (Join-Path $OutputDirectory 'fixture\StackCounterFixture.csproj')
...
dotnet build (Join-Path $OutputDirectory 'fixture\StackCounterFixture.csproj') -c Release --nologo -v quiet
if($LASTEXITCODE -ne 0){throw 'Fixture build failed.'}
$inputFile=Join-Path $OutputDirectory 'input\StackCounterFixture.exe'
dotnet run --project (Join-Path $OutputDirectory 'emitter\Emitter.csproj') -c Release -- (Join-Path $OutputDirectory 'fixture\bin\Release\net48\StackCounterFixture.exe') $inputFile
...
foreach($threads in 1,4){
 ...
 $project=Get-ChildItem $export -Recurse -Filter StackCounterFixture.csproj | Select-Object -First 1
 $rebuilt=Join-Path $OutputDirectory "rebuilt-$threads"
 dotnet build $project.FullName -c Release -o $rebuilt --nologo -v quiet
 if($LASTEXITCODE -ne 0){throw 'Source build failed.'}
 & (Join-Path $rebuilt 'StackCounterFixture.exe')
...
Write-Host 'PASS: stack counter regression'
