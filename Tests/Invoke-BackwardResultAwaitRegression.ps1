...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'BackwardResultAwaitFixture.cs') -Destination $inputDirectory
'<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFramework>net48</TargetFramework><OutputType>Exe</OutputType><Optimize>true</Optimize><LangVersion>latest</LangVersion></PropertyGroup></Project>' | Set-Content (Join-Path $inputDirectory 'BackwardResultAwaitFixture.csproj')
dotnet build $inputDirectory -c Release --nologo -v quiet
if($LASTEXITCODE -ne 0){throw 'Backward-result await fixture build failed.'}
$original=Join-Path $inputDirectory 'bin\Release\net48\BackwardResultAwaitFixture.exe'
$hash=(Get-FileHash -LiteralPath $original).Hash
$expected=@(& $original)
if($LASTEXITCODE -ne 0){throw 'Original backward-result await behavior failed.'}
...
foreach($threads in @(1,4)) {
    ...
    if($LASTEXITCODE -ne 0){throw 'Backward-result await source compilation failed.'}
    $actual=@(& (Join-Path $rebuilt 'BackwardResultAwaitFixture.exe'))
    if($LASTEXITCODE -ne 0){throw 'Backward-result await rebuilt behavior failed.'}
    $actual | Set-Content (Join-Path $OutputDirectory "actual-$threads.txt")
    if(Compare-Object $expected $actual -SyncWindow 0 -CaseSensitive){throw 'Backward-result await behavior changed.'}
}
$debug=Join-Path $OutputDirectory 'debug'
New-Item -ItemType Directory -Path $debug | Out-Null
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'BackwardResultAwaitDebug.cs') -Destination $debug
...
if($LASTEXITCODE -ne 0){throw 'Backward-result await safety/debug checks failed.'}
