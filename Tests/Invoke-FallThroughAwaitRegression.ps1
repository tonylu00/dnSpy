...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'FallThroughAwaitFixture.cs') -Destination $inputDirectory
'<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFramework>net48</TargetFramework><OutputType>Exe</OutputType><Optimize>true</Optimize><LangVersion>latest</LangVersion></PropertyGroup></Project>' | Set-Content (Join-Path $inputDirectory 'FallThroughAwaitFixture.csproj')
dotnet build $inputDirectory -c Release --nologo -v quiet
if($LASTEXITCODE -ne 0){throw 'Fallthrough await fixture build failed.'}
$original=Join-Path $inputDirectory 'bin\Release\net48\FallThroughAwaitFixture.exe'
$hash=(Get-FileHash -LiteralPath $original).Hash
$expected=@(& $original)
if($LASTEXITCODE -ne 0){throw 'Original fallthrough await behavior failed.'}
...
foreach($threads in @(1,4)) {
    ...
    if($LASTEXITCODE -ne 0){throw 'Fallthrough await export failed.'}
    $source=Get-Content (Join-Path $export 'FallThroughAwaitFixture\FallThroughAwaitFixture.cs') -Raw
    if($source -notmatch 'async Task<int> Read\('){throw 'Fallthrough loop was not reconstructed.'}
    $project=Get-ChildItem -LiteralPath $export -Recurse -Filter '*.csproj' | Select-Object -First 1
    $rebuilt=Join-Path $OutputDirectory "rebuilt-$threads"
    dotnet build $project.FullName -c Release -o $rebuilt --nologo -v quiet
    if($LASTEXITCODE -ne 0){throw 'Fallthrough await source compilation failed.'}
    $actual=@(& (Join-Path $rebuilt 'FallThroughAwaitFixture.exe'))
    if($LASTEXITCODE -ne 0){throw 'Fallthrough await rebuilt behavior failed.'}
    $actual | Set-Content (Join-Path $OutputDirectory "actual-$threads.txt")
    if(Compare-Object $expected $actual -SyncWindow 0 -CaseSensitive){throw 'Fallthrough await behavior changed.'}
}
$debug=Join-Path $OutputDirectory 'debug'
New-Item -ItemType Directory -Path $debug | Out-Null
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'FallThroughAwaitDebug.cs') -Destination $debug
...
if($LASTEXITCODE -ne 0){throw 'Fallthrough await safety/debug checks failed.'}
