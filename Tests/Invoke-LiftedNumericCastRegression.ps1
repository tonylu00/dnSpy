...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'LiftedNumericCastFixture.cs') -Destination $inputDirectory
'<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFramework>net48</TargetFramework><OutputType>Exe</OutputType><Optimize>true</Optimize><LangVersion>latest</LangVersion></PropertyGroup></Project>' | Set-Content (Join-Path $inputDirectory 'LiftedNumericCastFixture.csproj')
dotnet build $inputDirectory -c Release --nologo -v quiet
if($LASTEXITCODE -ne 0){throw 'Lifted numeric conversion fixture build failed.'}
$original=Join-Path $inputDirectory 'bin\Release\net48\LiftedNumericCastFixture.exe'
...
foreach($threads in @(1,4)) {
    ...
    if($LASTEXITCODE -ne 0){throw 'Lifted numeric conversion export failed.'}
    $source=Get-Content (Join-Path $export 'LiftedNumericCastFixture\LiftedNumericCastFixture.cs') -Raw
    $project=Get-ChildItem -LiteralPath $export -Recurse -Filter '*.csproj' | Select-Object -First 1
    $rebuilt=Join-Path $OutputDirectory "rebuilt-$threads"
    dotnet build $project.FullName -c Release -o $rebuilt --nologo -v quiet
    if($LASTEXITCODE -ne 0){throw 'Lifted numeric conversion source compilation failed.'}
    $actual=@(& (Join-Path $rebuilt 'LiftedNumericCastFixture.exe'))
    if($LASTEXITCODE -ne 0){throw 'Lifted numeric conversion rebuilt behavior failed.'}
    $actual | Set-Content (Join-Path $OutputDirectory "actual-$threads.txt")
    if(Compare-Object $expected $actual -SyncWindow 0 -CaseSensitive){throw 'Lifted numeric conversion behavior changed.'}
}
$debug=Join-Path $OutputDirectory 'debug'
New-Item -ItemType Directory -Path $debug | Out-Null
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'LiftedNumericCastDebug.cs') -Destination $debug
...
if($LASTEXITCODE -ne 0){throw 'Lifted numeric conversion safety/debug checks failed.'}
