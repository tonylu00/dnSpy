...
'@)
$source.ToString() | Set-Content (Join-Path $inputDirectory 'Program.cs')
...
$timer.Stop()
[pscustomobject]@{ Count=$Count; ExportSeconds=$timer.Elapsed.TotalSeconds; ExitCode=$exportCode } |
