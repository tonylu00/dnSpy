...
public static class IndependentAwaitCatchFixture {
    ...
    public static async Task<int> Sequential(Func<int, Task<int>> step, Action<Exception> observe, bool rethrow, bool branch) {
        try { await step(0).ConfigureAwait(false); }
        catch (Exception error) { observe(error); await step(1).ConfigureAwait(false); observe(error); if (rethrow) throw; }
        try { await step(2).ConfigureAwait(false); }
        catch (Exception error) { observe(error); await step(3).ConfigureAwait(false); observe(error); if (rethrow) throw; }
        return 17;
    }
    public static async Task<int> Exclusive(Func<int, Task<int>> step, Action<Exception> observe, bool rethrow, bool branch) {
        if (branch) {
            try { await step(0).ConfigureAwait(false); }
            catch (Exception error) { observe(error); await step(1).ConfigureAwait(false); observe(error); if (rethrow) throw; }
        } else {
    ...
    public static int Main() {
        var method = new DynamicMethod("ThrowPayload", typeof(void), new[] { typeof(object) }, typeof(IndependentAwaitCatchFixture).Module);
        ...
        for (int code = 0; code < 625; code++) foreach (int delayed in new[] { 0, 1, 2, 4, 8, 15 }) {
            ...
            int failure = -1;
            foreach (int body in kind == 0 ? new[] { 0, 2 } : kind == 1 ? new[] { 0 } : new[] { 2 }) {
                expected.Add(body);
                if (modes[body] == 0) continue;
            ...
            var task = kind == 0 ? Sequential(step, observed.Add, rethrow, false) : Exclusive(step, observed.Add, rethrow, kind == 1);
            ...
            if (failure < 0) Check(actual == null && result == 17, "Recovered result");
        ...
        Console.WriteLine("PASS: " + cases + " independent catch cases / " + checks + " assertions.");
        return 0;
    }
