...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'BitwiseComplementFixture.cs') -Destination $inputDirectory
'<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFramework>net48</TargetFramework><OutputType>Exe</OutputType><Optimize>true</Optimize><LangVersion>latest</LangVersion></PropertyGroup></Project>' | Set-Content (Join-Path $inputDirectory 'BitwiseComplementFixture.csproj')
dotnet build $inputDirectory -c Release --nologo -v quiet
if($LASTEXITCODE -ne 0){throw 'Complement fixture build failed.'}
$original=Join-Path $inputDirectory 'bin\Release\net48\BitwiseComplementFixture.exe'
...
foreach($threads in @(1,4)) {
    ...
    if($LASTEXITCODE -ne 0){throw 'Complement export failed.'}
    $source=Get-Content (Join-Path $export 'BitwiseComplementFixture\BitwiseComplementFixture.cs') -Raw
    $project=Get-ChildItem -LiteralPath $export -Recurse -Filter '*.csproj' | Select-Object -First 1
    $rebuilt=Join-Path $OutputDirectory "rebuilt-$threads"
    dotnet build $project.FullName -c Release -o $rebuilt --nologo -v quiet
    if($LASTEXITCODE -ne 0){throw 'Complement source compilation failed.'}
    $actual=@(& (Join-Path $rebuilt 'BitwiseComplementFixture.exe'))
    if($LASTEXITCODE -ne 0){throw 'Complement rebuilt behavior failed.'}
    $actual | Set-Content (Join-Path $OutputDirectory "actual-$threads.txt")
    if(Compare-Object $expected $actual -SyncWindow 0 -CaseSensitive){throw 'Complement behavior changed.'}
}
$debug=Join-Path $OutputDirectory 'debug'
New-Item -ItemType Directory -Path $debug | Out-Null
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'BitwiseComplementDebug.cs') -Destination $debug
...
if($LASTEXITCODE -ne 0){throw 'Complement safety/debug checks failed.'}
