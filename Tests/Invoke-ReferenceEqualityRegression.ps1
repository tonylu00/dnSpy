...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'ReferenceEqualityFixture.cs') -Destination $inputRoot
...
$inputExe=Join-Path $inputRoot 'bin\Release\net48\Fixture.exe'
...
if((Get-FileHash -LiteralPath $inputExe).Hash -ne $hash){throw 'Input changed.'}
