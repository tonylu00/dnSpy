...
public static partial class AsyncLayoutFixture {
    ...
    [AsyncStateMachine(typeof(DetachedBranchedState))]
    public static Task<int> ReadDetachedBranched(Task<int> input) {
        var machine = new DetachedBranchedState();
        machine.builder = AsyncTaskMethodBuilder<int>.Create();
        machine.input = input;
        machine.state = -1;
        machine.builder.Start(ref machine);
    ...
    [CompilerGenerated]
    sealed class DetachedBranchedState : IAsyncStateMachine {
        public int state;
        public AsyncTaskMethodBuilder<int> builder;
        public Task<int> input;
        TaskAwaiter<int> saved;
    ...
    static void VerifyDetachedAwaits() {
        ...
        Verify(ReadDetachedBranched);
        if (detachedFactories != 16) throw new Exception("Detached factory evaluation count");
        if (ReadDetachedPositive(null).GetAwaiter().GetResult() != 31 ||
            ReadDetachedNegative(null).GetAwaiter().GetResult() != 31 ||
            ReadDetachedForward(null).GetAwaiter().GetResult() != 31 ||
            ReadDetachedBranched(null).GetAwaiter().GetResult() != 31 || detachedFactories != 16)
