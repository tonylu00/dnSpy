...
if($LASTEXITCODE -ne 0){throw 'Grouped await-catch fixture build failed.'}
...
$expected=@(& $original)
if($LASTEXITCODE -ne 0){throw 'Original grouped catch behavior failed.'}
...
foreach($threads in @(1,4)) {
    ...
    if($LASTEXITCODE -ne 0){throw 'Grouped catch export failed.'}
    $project=Get-ChildItem -LiteralPath $export -Recurse -Filter '*.csproj' | Select-Object -First 1
    $rebuilt=Join-Path $OutputDirectory "rebuilt-$threads"
    dotnet build $project.FullName -c Release -o $rebuilt --nologo -v quiet
    if($LASTEXITCODE -ne 0){throw 'Grouped catch source compilation failed.'}
    $actual=@(& (Join-Path $rebuilt 'GroupedAwaitCatchFixture.exe'))
    if($LASTEXITCODE -ne 0){throw 'Grouped catch rebuilt behavior failed.'}
    $actual | Set-Content (Join-Path $OutputDirectory "actual-$threads.txt")
    if(Compare-Object $expected $actual -SyncWindow 0 -CaseSensitive){throw 'Grouped catch behavior changed.'}
}
$debug=Join-Path $OutputDirectory 'debug'
New-Item -ItemType Directory -Path $debug | Out-Null
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'GroupedAwaitCatchDebug.cs') -Destination $debug
...
if($LASTEXITCODE -ne 0){throw 'Grouped catch safety/debug checks failed.'}
