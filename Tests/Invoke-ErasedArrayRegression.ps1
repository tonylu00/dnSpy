...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'ErasedArrayFixture.cs') -Destination $inputDirectory
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'EmitErasedArrays.cs') -Destination $emitter
'<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFramework>net48</TargetFramework><OutputType>Exe</OutputType><Optimize>true</Optimize><LangVersion>latest</LangVersion></PropertyGroup></Project>' | Set-Content (Join-Path $inputDirectory 'ErasedArrayFixture.csproj')
...
dotnet build (Join-Path $inputDirectory 'ErasedArrayFixture.csproj') -c Release --nologo -v quiet
if($LASTEXITCODE -ne 0){throw 'Erased array fixture compilation failed.'}
$original=Join-Path $OutputDirectory 'ErasedArrayFixture.exe'
dotnet run --project (Join-Path $emitter 'Emitter.csproj') -c Release -- (Join-Path $inputDirectory 'bin\Release\net48\ErasedArrayFixture.exe') $original
if($LASTEXITCODE -ne 0){throw 'Erased array emission failed.'}
$expected=@(& $original)
if($LASTEXITCODE -ne 0){throw 'Emitted array behavior failed.'}
...
foreach($threads in @(1,4)) {
    ...
    if($LASTEXITCODE -ne 0){throw 'Erased array export failed.'}
    $project=Get-ChildItem -LiteralPath $export -Recurse -Filter '*.csproj' | Select-Object -First 1
    $rebuilt=Join-Path $OutputDirectory "rebuilt-$threads"
    dotnet build $project.FullName -c Release -o $rebuilt --nologo -v quiet
    if($LASTEXITCODE -ne 0){throw 'Erased array source compilation failed.'}
    $actual=@(& (Join-Path $rebuilt 'ErasedArrayFixture.exe'))
    if($LASTEXITCODE -ne 0){throw 'Erased array source behavior failed.'}
    $actual | Set-Content (Join-Path $OutputDirectory "actual-$threads.txt")
    if(Compare-Object $expected $actual -SyncWindow 0 -CaseSensitive){throw 'Erased array results differ.'}
}
$debug=Join-Path $OutputDirectory 'debug'
New-Item -ItemType Directory -Path $debug | Out-Null
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'ErasedArrayDebug.cs') -Destination $debug
...
if($LASTEXITCODE -ne 0){throw 'Array debug validation failed.'}
