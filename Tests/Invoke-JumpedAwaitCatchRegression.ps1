...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'JumpedAwaitCatchFixture.cs') -Destination $inputDirectory
'<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFramework>net48</TargetFramework><OutputType>Exe</OutputType><Optimize>true</Optimize><LangVersion>latest</LangVersion></PropertyGroup></Project>' | Set-Content (Join-Path $inputDirectory 'JumpedAwaitCatchFixture.csproj')
dotnet build $inputDirectory -c Release --nologo -v quiet
if($LASTEXITCODE -ne 0){throw 'Jumped await-catch fixture build failed.'}
$original=Join-Path $inputDirectory 'bin\Release\net48\JumpedAwaitCatchFixture.exe'
$hash=(Get-FileHash -LiteralPath $original).Hash
$expected=@(& $original)
if($LASTEXITCODE -ne 0){throw 'Original jumped catch behavior failed.'}
...
foreach($threads in @(1,4)) {
    ...
    if($LASTEXITCODE -ne 0){throw 'Jumped catch export failed.'}
    $project=Get-ChildItem -LiteralPath $export -Recurse -Filter '*.csproj' | Select-Object -First 1
    $rebuilt=Join-Path $OutputDirectory "rebuilt-$threads"
    dotnet build $project.FullName -c Release -o $rebuilt --nologo -v quiet
    if($LASTEXITCODE -ne 0){throw 'Jumped catch source compilation failed.'}
    $actual=@(& (Join-Path $rebuilt 'JumpedAwaitCatchFixture.exe'))
    if($LASTEXITCODE -ne 0){throw 'Jumped catch rebuilt behavior failed.'}
    $actual | Set-Content (Join-Path $OutputDirectory "actual-$threads.txt")
    if(Compare-Object $expected $actual -SyncWindow 0 -CaseSensitive){throw 'Jumped catch behavior changed.'}
}
$debug=Join-Path $OutputDirectory 'debug'
New-Item -ItemType Directory -Path $debug | Out-Null
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'JumpedAwaitCatchDebug.cs') -Destination $debug
...
if($LASTEXITCODE -ne 0){throw 'Jumped catch safety/debug checks failed.'}
