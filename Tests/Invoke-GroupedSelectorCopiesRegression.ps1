...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'GroupedAwaitCatchFixture.cs') -Destination $inputDirectory
'<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFramework>net48</TargetFramework><OutputType>Exe</OutputType><Optimize>true</Optimize><LangVersion>latest</LangVersion></PropertyGroup></Project>' | Set-Content (Join-Path $inputDirectory 'GroupedAwaitCatchFixture.csproj')
dotnet build $inputDirectory -c Release --nologo -v quiet
if($LASTEXITCODE -ne 0){throw 'Catch fixture build failed.'}
$original=Join-Path $inputDirectory 'bin\Release\net48\GroupedAwaitCatchFixture.exe'
...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'EmitGroupedSelectorCopies.cs') -Destination $emitter
...
$inputAssembly=Join-Path $variant 'GroupedAwaitCatchFixture.exe'
dotnet run --project (Join-Path $emitter 'Emitter.csproj') -c Release -- $original $inputAssembly
if($LASTEXITCODE -ne 0){throw 'Grouped selector rethrow emission failed.'}
...
foreach($threads in @(1,4)) {
    ...
    if($LASTEXITCODE -ne 0){throw 'Grouped selector rethrow export failed.'}
    $source=Get-Content (Join-Path $export 'GroupedAwaitCatchFixture\GroupedAwaitCatchFixture.cs') -Raw
    ...
    if($LASTEXITCODE -ne 0){throw 'Grouped selector rethrow source compilation failed.'}
    $actual=@(& (Join-Path $rebuilt 'GroupedAwaitCatchFixture.exe'))
    if($LASTEXITCODE -ne 0){throw 'Rebuilt grouped selector catch behavior failed.'}
    $actual | Set-Content (Join-Path $OutputDirectory "actual-$threads.txt")
    if(Compare-Object $expected $actual -SyncWindow 0 -CaseSensitive){throw 'Grouped selector catch behavior changed.'}
}
$debug=Join-Path $OutputDirectory 'debug'
New-Item -ItemType Directory -Path $debug | Out-Null
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'GroupedSelectorCopiesDebug.cs') -Destination $debug
...
if($LASTEXITCODE -ne 0){throw 'Grouped selector rethrow safety/debug checks failed.'}
