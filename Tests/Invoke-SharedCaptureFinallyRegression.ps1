...
if($LASTEXITCODE -ne 0){throw 'Shared capture rethrow emission failed.'}
...
foreach($threads in @(1,4)) {
    ...
    if($LASTEXITCODE -ne 0){throw 'Shared capture rethrow export failed.'}
    ...
    if($LASTEXITCODE -ne 0){throw 'Shared capture rethrow source compilation failed.'}
    $actual=@(& (Join-Path $rebuilt 'NestedAwaitFinallyFixture.exe'))
    if($LASTEXITCODE -ne 0){throw 'Rebuilt shared capture cleanup behavior failed.'}
    $actual | Set-Content (Join-Path $OutputDirectory "actual-$threads.txt")
    if(Compare-Object $expected $actual -SyncWindow 0 -CaseSensitive){throw 'Shared capture cleanup behavior changed.'}
}
$debug=Join-Path $OutputDirectory 'debug'
New-Item -ItemType Directory -Path $debug | Out-Null
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'SharedCaptureFinallyDebug.cs') -Destination $debug
...
if($LASTEXITCODE -ne 0){throw 'Shared capture rethrow safety/debug checks failed.'}
