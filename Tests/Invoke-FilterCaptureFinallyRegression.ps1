...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'FilterCaptureFinallyFixture.cs') -Destination $inputDirectory
'<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFramework>net48</TargetFramework><OutputType>Exe</OutputType><Optimize>true</Optimize><LangVersion>latest</LangVersion></PropertyGroup></Project>' | Set-Content (Join-Path $inputDirectory 'FilterCaptureFinallyFixture.csproj')
dotnet build $inputDirectory -c Release --nologo -v quiet
if($LASTEXITCODE -ne 0){throw 'Cleanup fixture build failed.'}
$original=Join-Path $inputDirectory 'bin\Release\net48\FilterCaptureFinallyFixture.exe'
...
$inputAssembly=Join-Path $variant 'FilterCaptureFinallyFixture.exe'
dotnet run --project (Join-Path $emitter 'Emitter.csproj') -c Release -- $original $inputAssembly 1 1
if($LASTEXITCODE -ne 0){throw 'Filter capture rethrow emission failed.'}
$variantHash=(Get-FileHash -LiteralPath $inputAssembly).Hash
$actual=@(& $inputAssembly)
if($LASTEXITCODE -ne 0 -or (Compare-Object $expected $actual -SyncWindow 0 -CaseSensitive)){throw 'Temporary sharing changed input behavior.'}
foreach($threads in @(1,4)) {
    ...
    if($LASTEXITCODE -ne 0){throw 'Filter capture rethrow export failed.'}
    $source=Get-Content (Join-Path $export 'FilterCaptureFinallyFixture\FilterCaptureFinallyFixture.cs') -Raw
    ...
    if($LASTEXITCODE -ne 0){throw 'Filter capture rethrow source compilation failed.'}
    $actual=@(& (Join-Path $rebuilt 'FilterCaptureFinallyFixture.exe'))
    if($LASTEXITCODE -ne 0){throw 'Rebuilt filter capture cleanup behavior failed.'}
    $actual | Set-Content (Join-Path $OutputDirectory "actual-$threads.txt")
    if(Compare-Object $expected $actual -SyncWindow 0 -CaseSensitive){throw 'Filter capture cleanup behavior changed.'}
}
$debug=Join-Path $OutputDirectory 'debug'
New-Item -ItemType Directory -Path $debug | Out-Null
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'FilterCaptureFinallyDebug.cs') -Destination $debug
...
if($LASTEXITCODE -ne 0){throw 'Filter capture rethrow safety/debug checks failed.'}
