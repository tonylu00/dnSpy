...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'EmptySiblingCatchFixture.cs') -Destination $inputDirectory
'<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFramework>net48</TargetFramework><OutputType>Exe</OutputType><Optimize>true</Optimize><LangVersion>latest</LangVersion></PropertyGroup></Project>' | Set-Content (Join-Path $inputDirectory 'EmptySiblingCatchFixture.csproj')
dotnet build $inputDirectory -c Release --nologo -v quiet
if($LASTEXITCODE -ne 0){throw 'Catch fixture build failed.'}
$original=Join-Path $inputDirectory 'bin\Release\net48\EmptySiblingCatchFixture.exe'
...
foreach($threads in @(1,4)) {
    ...
    if($LASTEXITCODE -ne 0){throw 'Empty sibling catch rethrow export failed.'}
    $source=Get-Content (Join-Path $export 'EmptySiblingCatchFixture\EmptySiblingCatchFixture.cs') -Raw
    ...
    if($LASTEXITCODE -ne 0){throw 'Empty sibling catch rethrow source compilation failed.'}
    $actual=@(& (Join-Path $rebuilt 'EmptySiblingCatchFixture.exe'))
    if($LASTEXITCODE -ne 0){throw 'Rebuilt empty sibling catch behavior failed.'}
    $actual | Set-Content (Join-Path $OutputDirectory "actual-$threads.txt")
    if(Compare-Object $expected $actual -SyncWindow 0 -CaseSensitive){throw 'Empty sibling catch behavior changed.'}
}
...
if($LASTEXITCODE -ne 0){throw 'Empty sibling catch rethrow safety/debug checks failed.'}
